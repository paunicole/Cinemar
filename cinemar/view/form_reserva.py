...
class FormularioReserva(Toplevel):
    def __init__(self, master = None, comprador = None, pelicula = None, base_datos = None):
        ...
        #Comprador
        self.Compr_label = ttk.Label(self.frame_datos)
        self.Compr_input = ttk.Entry(self.frame_datos)
        #Pelicula
        self.Peli_label = ttk.Label(self.frame_datos)
        self.Peli_input = ttk.Entry(self.frame_datos)
        #Fecha
        self.Fecha_label = ttk.Label(self.frame_datos)
        self.Fecha_input = ttk.Combobox(self.frame_datos)
        #Horario
        self.Hora_label = ttk.Label(self.frame_datos)
        self.Hora_input = ttk.Combobox(self.frame_datos)
        #Butacas Seleccionadas
        self.Butac_label = ttk.Label(self.frame_datos)
        self.Butac_input = ttk.Combobox(self.frame_datos)
        #Precio
        self.Precio_label = ttk.Label(self.frame_datos)
        self.Precio_entry = ttk.Entry(self.frame_datos)
        #Descuento
        self.Desc_label = ttk.Label(self.frame_datos)
        self.Desc_input = ttk.Entry(self.frame_datos)
        #Botones
        self.Reservar_bott = ttk.Button(self.frame_botones)
        self.Cancelar_bott = ttk.Button(self.frame_botones)
    ...
    def frames_config(self):
        self.frame_cabecera.config(border = 15, bg = '#002B40')
        self.frame_datos.config(bg = '#056595')
        self.frame_botones.config(bg = '#056595')
    ...
    def Reservar(self):
        ...
        if len(butac) > 0 and len(fecha) > 0 and len(hora) > 0:
            ...
            self.master.frame_reserva.fill_Tickets()
            self.Cancelar()
        else:
